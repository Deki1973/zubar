...
public interface AppointmentServiceInt extends AppointmentRepository {
    ...
    List<Appointment> findAllByOrderByAppointmentDateAndTimeDesc() throws AppointmentException;

    @Query(
            nativeQuery = true,
            value = "SELECT * FROM public.appointment pa WHERE pa.client_id=:clientId ORDER BY appointment_date_and_time DESC"
    )
    ...
    @Query(
            nativeQuery = true,
            value = "SELECT * FROM public.appointment pa WHERE pa.dentist_id=:dentistId ORDER BY appointment_date_and_time DESC"
    )
    ...
    value = "UPDATE appointment SET appointment_date_and_time=:newAppointmentDateAndTime, client_id=:clientId, dentist_id=:dentistId,description=:newDescription, completed=:completedVal, price=:price WHERE appointment_id=:appointmentId")
    int updateAppointment(Date newAppointmentDateAndTime, Long clientId, Long dentistId, String newDescription, Boolean completedVal, Long appointmentId, Double price) throws AppointmentException;
